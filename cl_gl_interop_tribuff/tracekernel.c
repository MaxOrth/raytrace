...
__kernel void trace(__write_only image2d_t destimg, const unsigned tricount, __read_only __global float *triangles, __read_only __global float4 *cam_mat)
{
  ...
  __global Tri *tri_buff = (__global Tri *)triangles;
  
  for (unsigned i = 0; i < tricount; ++i)
  {
    float3 a = tri_buff[i][0];
    float3 b = tri_buff[i][1];
    float3 c = tri_buff[i][2];
